...
async function fetchAllSquareCustomers(log) {
  ...
  do {
    // Square cursor pagination: cursor must be the only param when present
    const url = cursor
      ? `${SQUARE_BASE}/customers?cursor=${encodeURIComponent(cursor)}`
      : `${SQUARE_BASE}/customers?limit=100`;
...
if (require.main === module) {
  runSquareCustomerImport()
    .then(r => { console.log("Result:", JSON.stringify({ imported: r.imported, skipped: r.skipped })); process.exit(0); })
